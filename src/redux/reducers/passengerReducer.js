...
const passengerReducer = (state = initialState.passengers, { type, payload = [] }) => {
  switch (type) {
    case CREATE_PASSENGER_SUCCESS:
      return [...state, { ...payload }];
    case UPDATE_PASSENGER_SUCCESS:
      return state.map((passenger) => (passenger.id === payload.id ? payload : passenger));
    case LOAD_PASSENGERS_SUCCESS:
      ...
      if (!isPassport && !isDob && !isAddress) return response;
        ...
        return response.filter((ele) => {
          ...
          if ((isPassport && !ele.passport) || (isDob && !ele.dob) || (isAddress && !ele.address)) {
            return ele;
          }
        });
      }
