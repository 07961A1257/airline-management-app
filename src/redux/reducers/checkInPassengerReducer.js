...
const checkInPassengerReducer = (state = initialState.checkInPassengers, action) => {
  switch (action.type) {
    ...
    case types.DELETE_CHECKIN_PASSENGER_OPTIMISTIC:
      return state.filter((checkInPassenger) => checkInPassenger.id !== action.checkInPassenger.id);
